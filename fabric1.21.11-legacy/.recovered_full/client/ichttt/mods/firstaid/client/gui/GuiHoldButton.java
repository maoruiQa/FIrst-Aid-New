...
extends AbstractButton {
    ...
    public boolean mouseReleased(MouseButtonEvent event) {
        ...
        boolean bl = result = this.pressStart != -1L && super.mouseReleased(event);
        if (result) {
            this.pressStart = -1L;
        }
        return result;
    ...
    public int getTimeLeft() {
        if (this.pressStart == -1L) {
            return -1;
        }
        return (int)Math.max(0L, (long)this.holdTime - (Util.getMillis() - this.pressStart));
    }
