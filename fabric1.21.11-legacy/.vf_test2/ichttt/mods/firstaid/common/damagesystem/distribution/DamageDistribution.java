...
public abstract class DamageDistribution implements IDamageDistributionAlgorithm {
   public static float handleDamageTaken(
      ...
      if (left > 0.0F && redistributeIfLeft) {
         ...
         IDamageDistributionAlgorithm var12 = hasTriedNoKill ? RandomDamageDistributionAlgorithm.NEAREST_KILL : RandomDamageDistributionAlgorithm.getDefault();
         left = var12.distributeDamage(left, player, source, addStat);
         if (left > 0.0F && !hasTriedNoKill) {
            IDamageDistributionAlgorithm var13 = RandomDamageDistributionAlgorithm.NEAREST_KILL;
            left = var13.distributeDamage(left, player, source, addStat);
         }
      }
         ...
         if (damageModel == null) {
            ...
            for (Pair<EquipmentSlot, EnumPlayerPart[]> pair : this.getPartList()) {
               ...
               if (!Arrays.stream(parts).map(damageModel::getFromEnum).anyMatch(part -> part.currentHealth > this.minHealth(player, part))) {
                  ...
                  float absorbFactor = originalDamage / damage;
                  float damageDistributed = damage - damage;
