...
public final class PlayerModelRenderer {
   ...
   public static void renderPlayerHealth(
      int xOffset,
      int yOffset,
      ...
      if ((Boolean)FirstAidConfig.CLIENT.enableEasterEggs.get() && (EventCalendar.isAFDay() || EventCalendar.isHalloween())) {
         float renderAngle = angle;
         if (cooldown == 0) {
            renderAngle += (otherWay ? -partialTicks : partialTicks) * 2.0F;
         }

         if (FirstAidConfig.CLIENT.pos.get() != Position.BOTTOM_LEFT && FirstAidConfig.CLIENT.pos.get() != Position.TOP_LEFT) {
   ...
   private static int getColor(AbstractDamageablePart part, boolean fourColors, int opacity) {
      if (part.currentHealth <= 0.001F) {
         ...
         float percent = maxHealth <= 0.0F ? 0.0F : part.currentHealth / maxHealth;
         if (fourColors) {
            if (percent > 0.75F) {
               ...
               return percent > 0.25F ? ARGB.color(opacity, 245, 200, 40) : ARGB.color(opacity, 235, 70, 60);
            }
         } else if (percent > 0.5F) {
            return ARGB.color(opacity, 60, 220, 60);
         } else {
