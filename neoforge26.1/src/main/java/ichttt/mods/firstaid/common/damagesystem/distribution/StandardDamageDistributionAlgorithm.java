...
public class StandardDamageDistributionAlgorithm extends DamageDistribution {
    public static final MapCodec<StandardDamageDistributionAlgorithm> CODEC = RecordCodecBuilder.mapCodec(instance ->
            instance.group(
                    Codec.unboundedMap(EquipmentSlot.CODEC, StringRepresentable.fromEnum(() -> EnumPlayerPart.VALUES).listOf())
                            .fieldOf("partMap").forGetter(o -> o.builtList.stream().collect(Collectors.toMap(Pair::getLeft, pair -> Arrays.asList(pair.getRight())))),
